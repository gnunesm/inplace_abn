...
def save_log_output(logstring, log_time):
    f = open("output_batch_train/output_"+log_time+".txt","a+")
    f.write(logstring+"\n")
    f.close()

def save_preds(log_time, preds, img_timestamp):
    f= open("output_batch_train/preds_output_" +log_time+ ".txt","a+")
...
#    momentum = 0.98
 ...
 #          LR *= 0.1
        ...
        for batch_i, batch  in enumerate(train_loader):
            ...
            save_preds(log_time, preds, batch['img_timestamp'])
#            with torch.no_grad():
#                model.eval()
...
#                model.train()
            ...
            print(logstring)
            save_log_output(log_output, log_time)
